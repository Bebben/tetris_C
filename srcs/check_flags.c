...
** Last update Wed Mar 15 21:43:09 2017 benoit pingris
*/
...
int	check_commands(t_binding *t)
{
  if (my_strcmp(t->k_left, t->k_right) == 0 ||
      my_strcmp(t->k_left, t->k_turn) == 0 ||
      my_strcmp(t->k_left, t->k_pause) == 0 ||
      my_strcmp(t->k_left, t->k_drop) == 0 ||
      my_strcmp(t->k_right, t->k_turn) == 0 ||
      my_strcmp(t->k_right, t->k_pause) == 0 ||
      my_strcmp(t->k_right, t->k_drop) == 0 ||
      my_strcmp(t->k_turn, t->k_drop) == 0 ||
      my_strcmp(t->k_turn, t->k_pause) == 0 ||
      my_strcmp(t->k_drop, t->k_pause) == 0)
    return (ERROR);
  return (SUCCESS);
}
