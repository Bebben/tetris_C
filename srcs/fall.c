...
** Last update Sat Mar 18 12:27:14 2017 benoit pingris
*/
...
void	*my_controls(t_coord *c, t_tabs *s, t_binding k, t_coord my_c)
{
  char	*str;
...
char	**falling_2(t_tabs s, t_coord c, t_time t, t_binding k)
{
  ...
  while (check_stop(s.map) == 0)
      ...
      if (t.end_t - t.start_t > 1000 - k.level * 100)
	{
      ...
      s.map = falling(&c, s.map, k);
      my_controls(&c, &s, k, c);
      if (enlarge_map(k.map_col, k.map_row) == ERROR || c.ret == 5)
	{
	  endwin();
	  return (NULL);
	}
